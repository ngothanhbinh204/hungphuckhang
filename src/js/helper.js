...
export function ToggleItem() {
	...
	if (nodeTitle.length > 0) {
		const firstTitle = nodeTitle.first();
		const firstContent = firstTitle.next();
...
export function setOffsetToParent(item, parent) {
	const $item = $(item);
	const $parent = $(parent);

	if (!$item.length || !$parent.length) return;

	const offsetBottom =
		$parent.innerHeight() - ($item.position().top + $item.outerHeight());
...
export function replaceSvgImages() {
	$(".img-svg").each(function () {
		...
		// Check if the image is SVG, skip if it's PNG or JPG
		if (!imgURL) return;

		const extension = imgURL.split('.').pop().toLowerCase();
		if (extension === 'png' || extension === 'jpg' || extension === 'jpeg') {
			return; // Skip non-SVG images, let them display normally
		}
		...
		if (edgePosition === "inner") {
			if ($this.closest(".container") && $this.closest(".container").closest(".container-fluid")) {
				const newOffset = Math.abs(
					$(".wide-container-js").offset().left - $(".default-container-js").offset().left
				);
				offset = newOffset;
			}
		}
